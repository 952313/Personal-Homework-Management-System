...
class HomeworkPlatform:
    ...
    def create_widgets(self):
        ...
        self.settings_tab = self.tabview.add("设置")  # 新增设置选项卡
    ...
    def build_settings_tab(self, parent):
        ...
        font_frame.pack(fill="x", padx=50, pady=20)
        ...
        # 应用设置按钮
        apply_button = ctk.CTkButton(font_frame, text="应用字号设置", command=self.apply_font_settings,
                                    height=40, font=ctk.CTkFont(size=18, weight="bold"))
        apply_button.pack(pady=30)
        
        # 提示信息
        hint_label = ctk.CTkLabel(font_frame, 
                                 text="注意：应用设置后需要重启程序才能完全生效",
    ...
    def on_main_font_slider_change(self, value):
        """主界面字号滑块变化时的回调"""
        self.main_font_size_label.configure(text=str(int(float(value))))

    def on_table_font_slider_change(self, value):
        """表格字号滑块变化时的回调"""
        self.table_font_size_label.configure(text=str(int(float(value))))

    def apply_font_settings(self):
        """应用字号设置"""
        try:
            new_main_size = int(float(self.main_font_size_var.get()))
            new_table_size = int(float(self.table_font_size_var.get()))
            
            # 更新设置
            self.settings["main_font_size"] = new_main_size
            self.settings["table_font_size"] = new_table_size
            
            # 保存设置
            self.save_settings()
            
            messagebox.showinfo("成功", "字号设置已保存！\n\n部分设置需要重启程序才能完全生效。")
            
        except ValueError:
            messagebox.showerror("错误", "字号设置无效！")
    ...
    def build_about_tab(self, parent):
        ...
        # 版本信息
        version_label = ctk.CTkLabel(parent, text="版本 2.3", 
                                    font=ctk.CTkFont(size=18))
