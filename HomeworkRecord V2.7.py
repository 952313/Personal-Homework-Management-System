...
class HomeworkPlatform:
    ...
    def export_homework_text(self):
        ...
        for item in self.tree.get_children():
            ...
            if values:
                displayed_items.append(values)
        
        if not displayed_items:
            messagebox.showinfo("导出", "没有作业可导出")
            return
        
        # 定义每列的基准宽度（一个制表符约等于4个字符宽度）
        tab_width = 4
        column_widths = [10, 6, 40, 12, 12, 10]  # 代号, 科目, 内容, 创建日期, 截止日期, 状态
        
        # 创建导出窗口
        export_window = ctk.CTkToplevel(self.root)
        export_window.title("导出作业")
        export_window.geometry("900x600")
        export_window.transient(self.root)
        export_window.grab_set()
        
        # 标题
        title_label = ctk.CTkLabel(export_window, text="作业导出内容", 
                                  font=ctk.CTkFont(size=20, weight="bold"))
        title_label.pack(pady=10)
        
        # 创建文本框
        text_frame = ctk.CTkFrame(export_window)
        text_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        text_widget = ctk.CTkTextbox(text_frame, font=ctk.CTkFont(size=12, family="Consolas"))
        text_widget.pack(fill="both", expand=True, padx=10, pady=10)
        
        # 生成导出文本
        def calculate_tabs(text, target_width):
            """计算需要多少个制表符才能达到目标宽度"""
            text_width = len(str(text))
            if text_width >= target_width:
                return "\t"  # 至少一个制表符
            needed_tabs = (target_width - text_width) // tab_width + 1
            return "\t" * needed_tabs
        
        # 表头
        header_parts = []
        headers = ["代号", "科目", "内容", "创建日期", "截止日期", "状态"]
        for i, header in enumerate(headers):
            header_parts.append(header)
            if i < len(headers) - 1:
                header_parts.append(calculate_tabs(header, column_widths[i]))
        
        header = "".join(header_parts) + "\n"
        separator = "-" * 120 + "\n"
        export_text = header + separator
        
        # 数据行
        for values in displayed_items:
            line_parts = []
            for i, value in enumerate(values):
                line_parts.append(str(value))
                if i < len(values) - 1:
                    line_parts.append(calculate_tabs(value, column_widths[i]))
        ...
        def copy_to_clipboard():
            ...
            self.show_temp_message("已复制到剪贴板")
        
        copy_button = ctk.CTkButton(export_window, text="复制到剪贴板", 
    ...
    def build_main_tab(self, parent):
        ...
        ctk.CTkButton(button_frame, text="刷新列表", command=self.refresh_list,
                      height=35, font=ctk.CTkFont(size=self.settings["main_font_size"])).pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(button_frame, text="导出作业", command=self.export_homework_text,
                      height=35, font=ctk.CTkFont(size=self.settings["main_font_size"])).pack(fill="x", padx=10, pady=5)
